...
ClutterCutter is a single Rust application — a low-level Win32 GUI with no GUI
framework. CI builds it on every push and attaches it to every GitHub Release.

| Build | Source | Output | Status |
|-------|--------|--------|--------|
| **ClutterCutter** | `rust/` crate | `cluttercutter.exe` → shipped as `ClutterCutter.exe` | **The shipping build** — current UI + all features; **this is what winget packages** |

The native GUI (`cluttercutter` binary) is **Windows-only**. There's also a
cross-platform CLI harness (`cluttercutter-cli`); Linux support for the scan core
is in progress (a portable `std::fs` scan core now compiles for Linux), and a
cross-platform egui GUI is planned but not yet shipped.
...
- Ideas / candidate next work (none committed yet — confirm before building):
  ...
  - Ship a cross-platform egui GUI (Linux scan core already compiles).
